...
shady_cleanup_module(int devices_to_destroy)
{
  ...
  //
  // XXX RESTORE THE OLD OPEN
  //
  syscall_tableaddress[__NR_open] = old_open;
...
shady_init_module(void)
{
  ...
  //
  // XXX SAVE OLD OPEN CALL
  //
  old_open = syscall_tableaddress[__NR_open];
  syscall_tableaddress[__NR_open] = my_open;
